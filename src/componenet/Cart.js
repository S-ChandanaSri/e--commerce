...
/*async function processCart() {
  try {
    ...
    if (latestCartId !== undefined) {
      await removeItem(latestCartId); 
    } else {
      console.error('Latest Cart ID is undefined. Cannot delete the cart.');
    }
  } catch (error) {
...
const handleSubmit=()=>{
  ...
  setShowDown(false)
   ...
   /* const {
        ...
        if(isEmpty) return<h1 className='text-center'>yours cart is empty</h1>*/
  ...
  return (
    ...
    <div className="d-flex align-items-center justify-content-between position-fixed top-0 w-100" style={{backgroundColor: "#2874f0",borderRadius: "1rem"}}>
        <div className="left" style={{marginLeft: "5rem",display:"flex",alignItems: "center"}}>
            ...
            <div className="search" style={{display:"flex"}}  >
                    ...
                    <div className="recentsearch" style={{position:"absolute",zIndex:"1"}} >
        ...
        <div className="right" style={{display:"flex",alignItems: "center"}}>
           <a href="#">
            <button style={{marginRight: "5rem",padding:"0.5rem 2rem",backgroundColor: "white"}}>
                <Link to='/loogin' style={{textDecoration:"none",color:"black"}}>
                Login
                </Link>
                </button>
           </a>
   ...
   <div style={{padding:"0.5rem 10rem",border:"2px solid #F0F0F0"}} />
    ...
    <div  className='dat'  style={{position:"relative",top:"9rem"}}>
                ...
                <button style={{padding:"0.5rem 4.5rem",backgroundColor:"#FB641B",position:"relative",right:"2rem"}}>
                    <Link to="/loogin" style={{color:"white",textDecoration:"none"}}>
                    Login
                    </Link>
                    </button>
            </div>
            ...
            {showDown &&(
              ...
              <img alt="down"  className='groceries' src="https://rukminim2.flixcart.com/www/800/800/promos/16/05/2019/08dddd59-c0ff-4efd-8723-6d847f5df25f.png?q=90"  style={{height: "200px",width: "250px"}}/>
               <div><h2>Your basket is empty!</h2></div>
               <div style={{padding:"0.5rem 0rem"}}>Enjoy upto 50% saving on Grocery</div>
               <button style={{padding:"0.5rem 4rem",color:"white",backgroundColor:"#2874F0"}}>Shop Now</button>
...
/*
<div className="abs" >
    <a href="#" style={{fontSize:"larger"}} className="f" onClick={handleSubmit} >Flipkart</a>
    <a  href="#" className="ggg" style={{fontSize:"larger"}} tabIndex="0">Groceries</a>
...
<section className='py-4 container'>
  ...
  async  function getLatestCartId(){
    ...
    await fetch(`http://localhost:3001/car?userid=${userid}`)
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
...
async function removeItem(latestCartId){
  try{
const response = await axios.delete(`http://localhost:3001/carrt?cartid=${latestCartId}`);
if(response.data.status === "success"){
  console.log("item removed successfully");
